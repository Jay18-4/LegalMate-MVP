...
oayth2_bearer = OAuth2PasswordBearer(tokenUrl='auth/token')
...
async def create_user(db: db_dependency, create_user_request: CreateUserReqest):
    if db.query(User).filter(User.username == create_user_request.username).first():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail="Username already exists.")
    if db.query(User).filter(User.user_email == create_user_request.email).first():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
...
def authenticate_user(username: str, password: str, db: Session = Depends(get_db)):
    ...
    #email = db.query(User).filter(User.user_email == username).first()
...
async def get_current_user(token: Annotated[str, Depends(oayth2_bearer)]):
    try:
